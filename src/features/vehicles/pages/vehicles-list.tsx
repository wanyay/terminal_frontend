    ...
    <>
      ...
      <Main>
        <div className='mb-4 flex flex-wrap items-center justify-between gap-4'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>{t('vehicles.title' as never)}</h1>
            <p className='text-muted-foreground text-sm'>{t('vehicles.subtitle' as never)}</p>
          </div>
          <Button onClick={() => setFormOpen(true)}>
            <Plus className='mr-2 h-4 w-4' />{t('vehicles.registerEntry' as never)}
          </Button>
        </div>
