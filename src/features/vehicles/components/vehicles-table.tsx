...
const statusVariant: Record<string, 'default' | 'secondary' | 'destructive'> = {
  ENTERED: 'default', EXITED: 'secondary', CANCELLED: 'destructive',
}

const columns: ColumnDef<Vehicle>[] = [
  { accessorKey: 'plateNumber', header: 'Plate', cell: ({ row }) => <span className='font-mono font-medium'>{row.getValue('plateNumber')}</span> },
  { accessorKey: 'visitorName', header: 'Visitor', cell: ({ row }) => <span>{row.getValue('visitorName')}</span> },
  {
    accessorKey: 'vehicleType', header: 'Type', cell: ({ row }) => {
  ...
  {
    id: 'actions', header: 'Actions', cell: function ActionCell({ row }) {
      ...
      return (
        <>
          <div className='flex items-center gap-1'>
            ...
            {v.status === 'ENTERED' && (
              <Button variant='ghost' size='icon' className='h-8 w-8' onClick={() => doExit({ id: v.id, exitGateId: v.entryGateId || '' })} disabled={actionPending} title='Exit'><LogOut className='h-4 w-4 text-blue-600' /></Button>
            )}
...
export function VehiclesTable({ page, perPage, search, onPageChange, onPerPageChange, onSearchChange }: Props) {
  ...
  if (isError) return <div className='flex items-center justify-center py-12'><p className='text-destructive text-sm'>Failed to load vehicles.</p></div>

  return (
    <div className='space-y-4'>
      <Input placeholder='Search vehicles...' value={search} onChange={(e) => { onSearchChange(e.target.value); onPageChange(1) }} className='h-8 w-[250px]' />
      <div className='rounded-md border'>
        <Table>
          ...
          <TableBody>
            ...
            : vehicles.length === 0 ? (<TableRow><TableCell colSpan={columns.length} className='h-24 text-center text-muted-foreground'>No vehicles found.</TableCell></TableRow>)
